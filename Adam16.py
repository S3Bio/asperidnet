...
#         print('Epoch: {}, accuracy = {:.2f}'.format(epoch, num_correct / num_examples))
    ...
    def create_model():
        ...
        for i in range(n_layers):
            output_size = trial.suggest_int('n_units_l{}'.format(i), 160, 190)
...
study = optuna.create_study(study_name='Adam_16', storage='sqlite:///irstudy16_2.db', load_if_exists=True, direction="maximize")
study.optimize(func, n_trials=1000)
df_results = study.trials_dataframe(attrs=('number', 'value', 'params', 'state'))
df_results.to_pickle('Adam_16.pkl')
df_results.to_csv('Adam_16.csv')
